...
const ICON_FALLBACK_DATA_URI =
  ...
  encodeURIComponent(ICON_FALLBACK_SVG);
...
function resolveUrl(possibleHref: string, baseUrl: string): string {
  try {
...
async function fetchPageHtml(url: string, timeoutMs = 6000): Promise<string | null> {
  ...
  try {
    const res = await fetch(url, {
      ...
      redirect: "follow",
      headers: {
        // Some sites respond differently without a UA.
...
function extractPageTitle(html: string): string | null {
  // Prefer the actual <title> tag. If not available, fall back to og:title.
...
function extractFavicon(html: string, baseUrl: string): string | null {
  // Gather likely favicon link tags, then take the first with an href.
  // Examples:
  ...
  for (const tag of iconLinks) {
    ...
    // Some sites put data URIs or weird values; prefer URL-ish ones.
...
export async function resolveIconForUrl(inputUrl: string): Promise<string> {
  ...
  // If we couldn't parse, still return a reasonable "favicon" guess.
...
export async function resolveIconAndTitleForUrl(
  inputUrl: string,
): Promise<{ iconUrl: string; pageTitle: string | null }> {
  const url = normalizeUrl(inputUrl);
  if (!url) {
...
export function displayTitleForUrl(
  url: string,
  title?: string,
