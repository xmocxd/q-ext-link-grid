...
function getCached(urlKey) {
  ...
  return {
    iconUrl: entry.iconUrl == null ? null : String(entry.iconUrl),
...
async function resolveIconAndTitleForUrlUncached(url) {
  if (!url) {
    return { iconUrl: null, pageTitle: null };
  }
  ...
  if (html) {
    ...
    return {
      iconUrl: favicon ?? ogImage ?? fallbackFavicon ?? null,
      pageTitle,
    };
  }
